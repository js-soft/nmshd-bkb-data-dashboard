...
if ! sqlcmd -S $MSSQL_HOST,$MSSQL_PORT -U $MSSQL_USER -P $MSSQL_PASSWORD -C -Q "SELECT 1" &> /dev/null; then
    echo "Connection to database at $MSSQL_HOST:$MSSQL_PORT failed for user $MSSQL_USER. Please check your connection settings."
    exit 1
fi
